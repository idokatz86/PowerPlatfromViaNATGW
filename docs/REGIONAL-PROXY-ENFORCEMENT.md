...
| Direct connector paths | Direct calls to `api.ipify.org` and `checkip.amazonaws.com` already proved they can show `<microsoft-managed-egress-ip>`, not the NAT IPs. |
...
- AWS sees stable customer-controlled source IPs: `<north-region-nat-ip>` and `<west-region-nat-ip>` in this demo.
...
| --- | --- |
| North Europe proxy proof | `api.ipify.org` and `checkip.amazonaws.com` observe `<north-region-nat-ip>`. |
| West Europe proxy proof | `api.ipify.org` and `checkip.amazonaws.com` observe `<west-region-nat-ip>`. |
...
| AWS allowlist | AWS allows only `<north-region-nat-ip>` and `<west-region-nat-ip>` for this integration path. |
